...
#Loading packages
Here we are loading packages that will be of use to us, for more information refre to the stat aanalaysis section
```{r}
...
##QQPLot
Here we creat a QQ plot to check the linear model assumptions fo the random efffect
```{r}
...
##Light Wrangling
Here we are doing osme light wrangling before moving on to our final plotting, but only needed if we decde to present the data in a way displaying "trt + day" together
```{r}
...
#Final Plot
Here we bring in our intitial wrangled data to create boxplots with error bars, we set our dependednt variable of mg_kg_tin to the y axis and our independent variables of day and treatment to the y axis, displaying both through the use of the facet_grid fucntion. Our letters our created trough our emmean values we created a few steps above, then we save it in a size that makes it legible.
